package com.wms.web.shop.controller.order;
...
import com.wms.core.business.order.service.orderproduct.OrderProductDownloadService;
import com.wms.core.business.payments.model.PaymentMethod;
...
import com.wms.web.admin.entity.userpassword.UserReset;
...
import com.wms.web.populator.order.ReadableOrderTotalPopulator;
import com.wms.web.populator.order.ReadableShippingSummaryPopulator;
import com.wms.web.populator.order.ReadableShopOrderPopulator;
...
import com.wms.web.shop.controller.shoppingCart.facade.ShoppingCartFacade;
...
public class ShoppingOrderController extends AbstractController {
	...
	public String displayCheckout(@CookieValue("cart") String cookie, Model model, HttpServletRequest request, HttpServletResponse response, Locale locale) throws Exception {
		...
		com.wms.core.business.shoppingcart.model.ShoppingCart cart = null;
		...
		if (customer != null) {
			 ...
			 * (AnonymousCustomer)request.getAttribute
			 * (Constants.ANONYMOUS_CUSTOMER); if(anonymousCustomer!=null &&
			 * anonymousCustomer.getBilling()!=null) { Billing billing =
			 * customer.getBilling();
			 * billing.setCity(anonymousCustomer.getBilling().getCity());
			 * Map<String,Country> countriesMap =
			 * countryService.getCountriesMap(language); Country
			 * anonymousCountry =
			 * countriesMap.get(anonymousCustomer.getBilling().getCountry());
			 * if(anonymousCountry!=null) {
			 * billing.setCountry(anonymousCountry); } Map<String,Zone> zonesMap
			 * = zoneService.getZones(language); Zone anonymousZone =
			 * zonesMap.get(anonymousCustomer.getBilling().getZone());
			 * if(anonymousZone!=null) { billing.setZone(anonymousZone); }
			 * if(anonymousCustomer.getBilling().getPostalCode()!=null) {
			 * billing
			 * .setPostalCode(anonymousCustomer.getBilling().getPostalCode()); }
			 * customer.setBilling(billing); }
			 * order.setCustomer(anonymousCustomer);
			 */
		}
	...
	public String commitOrder(@CookieValue("cart") String cookie, @Valid @ModelAttribute(value = "order") ShopOrder order, BindingResult bindingResult, Model model, HttpServletRequest request,
	...
	public @ResponseBody ReadableShopOrder calculateShipping(@ModelAttribute(value = "order") ShopOrder order, HttpServletRequest request, HttpServletResponse response, Locale locale)
			throws Exception {
		...
		try {
			...
			// re-generate cart
			com.wms.core.business.shoppingcart.model.ShoppingCart cart = shoppingCartFacade.getShoppingCartModel(shoppingCartCode, store);
	...
	public @ResponseBody ReadableShopOrder calculateOrderTotal(@ModelAttribute(value = "order") ShopOrder order, HttpServletRequest request, HttpServletResponse response, Locale locale)
			throws Exception {
