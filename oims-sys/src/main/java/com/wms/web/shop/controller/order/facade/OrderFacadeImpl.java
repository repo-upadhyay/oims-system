...
import com.wms.web.entity.order.OrderEntity;
...
import com.wms.web.entity.order.ReadableOrderProduct;
import com.wms.web.entity.order.ShopOrder;
import com.wms.web.populator.customer.CustomerPopulator;
import com.wms.web.populator.customer.PersistableCustomerPopulator;
import com.wms.web.populator.order.OrderProductPopulator;
import com.wms.web.populator.order.ReadableOrderPopulator;
import com.wms.web.populator.order.ReadableOrderProductPopulator;
import com.wms.web.populator.order.ShoppingCartItemPopulator;
...
public class OrderFacadeImpl implements OrderFacade {
	...
	private void setOrderTotals(OrderEntity order, OrderTotalSummary summary) {
		...
		List<com.wms.core.business.order.model.OrderTotal> orderTotals = summary.getTotals();
		for(com.wms.core.business.order.model.OrderTotal t : orderTotals) {
	...
	private Order processOrderModel(ShopOrder order, Customer customer, Transaction transaction, MerchantStore store,
		...
		try {
			...
			List<com.wms.core.business.order.model.OrderTotal> totals = summary.getTotals();

			//re-order totals
			Collections.sort(
					totals,
					new Comparator<com.wms.core.business.order.model.OrderTotal>() {
					       public int compare(com.wms.core.business.order.model.OrderTotal x, com.wms.core.business.order.model.OrderTotal y) {
			...
			Set<com.wms.core.business.order.model.OrderTotal> modelTotals = new LinkedHashSet<com.wms.core.business.order.model.OrderTotal>();
			for(com.wms.core.business.order.model.OrderTotal total : totals) {
			...
			if(PaymentType.PAYPAL.name().equals(paymentType)) {
				...
				payment = new com.wms.core.business.payments.model.PaypalPayment();
				
				((com.wms.core.business.payments.model.PaypalPayment)payment).setPayerId(transaction.getTransactionDetails().get("PAYERID"));
				((com.wms.core.business.payments.model.PaypalPayment)payment).setPaymentToken(transaction.getTransactionDetails().get("TOKEN"));
