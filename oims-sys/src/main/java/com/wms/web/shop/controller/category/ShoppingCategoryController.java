package com.wms.web.shop.controller.category;
...
import com.wms.web.entity.catalog.ProductList;
...
import com.wms.web.entity.shop.Breadcrumb;
import com.wms.web.entity.shop.PageInformation;
import com.wms.web.populator.catalog.ReadableCategoryPopulator;
import com.wms.web.populator.catalog.ReadableProductPopulator;
import com.wms.web.populator.manufacturer.ReadableManufacturerPopulator;
...
import com.wms.web.utils.BreadcrumbsUtils;
import com.wms.web.utils.LabelUtils;
import com.wms.web.utils.PageBuilderUtils;
...
public class ShoppingCategoryController {
	...
	private List<ReadableManufacturer> getManufacturers(MerchantStore store, List<Long> ids, Language language) throws Exception {
		List<ReadableManufacturer> manufacturerList = null;
		List<com.wms.core.business.catalog.product.model.manufacturer.Manufacturer> manufacturers = manufacturerService.listByProductsByCategoriesId(store, ids, language);
		if(!CollectionUtils.isEmpty(manufacturers)) {
			manufacturerList = new ArrayList<ReadableManufacturer>();
			for(com.wms.core.business.catalog.product.model.manufacturer.Manufacturer manufacturer : manufacturers) {
	...
	private List<ReadableCategory> getSubCategories(MerchantStore store, Category category, Map<Long,Long> productCount, Language language, Locale locale) throws Exception {
		...
		for(Category sub : subCategories) {
			...
			//com.wms.web.entity.catalog.Category cProxy =  catalogUtils.buildProxyCategory(sub, store, locale);
	 ...
	 * url example :  http://<host>/oims-sys/shop/services/public/products/DEFAULT/BOOKS
	 * @param store
	 * @param language
	 * @param category
	 * @param model
	...
	public ProductList getProducts(@PathVariable final String store, @PathVariable final String language, @PathVariable final String category, Model model, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		//http://localhost:8080/oims-sys/services/public/products/DEFAULT/en/book.html
		...
		try {
			...
			List<com.wms.core.business.catalog.product.model.Product> products = productService.getProducts(ids, lang);
