...
import com.wms.core.business.catalog.product.model.image.ProductImageDescription;
...
public class ProductController {
	...
	private String displayProduct(Long productId, Model model, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		com.wms.web.admin.entity.catalog.Product product = new com.wms.web.admin.entity.catalog.Product();
	...
	public String saveProduct(@Valid @ModelAttribute("product") com.wms.web.admin.entity.catalog.Product  product, BindingResult result, Model model, HttpServletRequest request, Locale locale) throws Exception {
		...
		//sku validate
		if (newProduct==null && productService.getByCode(product.getProduct().getSku().toString(), language) != null) {
			ObjectError error = new ObjectError("sku", "SKU is already available. Please use different SKU code.");
			result.addError(error);
		}
